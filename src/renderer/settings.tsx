...
type DashboardSortMode = 'attention' | 'local-activity' | 'remote-activity';
type DashboardNotifSort = 'count' | 'name';

interface DashboardPreferences {
  dashboardDefaultFilter?: DashboardCardFilter;
  dashboardSortMode?: DashboardSortMode;
  dashboardNotifSort?: DashboardNotifSort;
}
...
function DashboardSettingsSection() {
  ...
  const loadPrefs = async () => {
    try {
      const allPrefs = await window.jarvis.getPreferences();
      setPrefs({
        dashboardDefaultFilter: allPrefs.dashboardDefaultFilter,
        dashboardSortMode: allPrefs.dashboardSortMode,
        dashboardNotifSort: allPrefs.dashboardNotifSort,
      });
    } catch (err) {
      console.error('[Settings] Failed to load dashboard preferences:', err);
    }
  };
  ...
  const handleSave = async () => {
    ...
    setSaved(false);
    try {
      await window.jarvis.setPreferences({
        dashboardDefaultFilter: prefs.dashboardDefaultFilter,
        dashboardSortMode: prefs.dashboardSortMode,
        dashboardNotifSort: prefs.dashboardNotifSort,
      });
      setSaved(true);
    } catch (err) {
      console.error('[Settings] Failed to save dashboard preferences:', err);
    } finally {
      setSaving(false);
    }
  };
  ...
  const cardFilterOptions = [
    { value: 'all' as DashboardCardFilter, label: 'All Repos', description: 'Show all repos by default' },
    { value: 'healthy' as DashboardCardFilter, label: 'Healthy', description: 'Show only repos without warnings' },
    { value: 'warnings' as DashboardCardFilter, label: 'Need Attention', description: 'Show repos with branch or remote warnings' },
    { value: 'notifications' as DashboardCardFilter, label: 'With Notifications', description: 'Show repos that have unread notifications' },
    { value: 'human-notifications' as DashboardCardFilter, label: 'People First', description: 'Show notifications from other users' },
    { value: 'failed-runs' as DashboardCardFilter, label: 'Failed Runs', description: 'Show repos with failed workflow runs' },
  ];

  const sortModeOptions = [
    { value: 'attention' as DashboardSortMode, label: 'Attention First', description: 'Repos with warnings first, then alphabetical' },
    { value: 'local-activity' as DashboardSortMode, label: 'Local Activity', description: 'Most recently committed repos first' },
    { value: 'remote-activity' as DashboardSortMode, label: 'Remote Activity', description: 'Most recently pushed repos first' },
  ];

  const notifSortOptions = [
    { value: 'count' as DashboardNotifSort, label: 'By Count', description: 'Sort notifications by count' },
    { value: 'name' as DashboardNotifSort, label: 'By Name', description: 'Sort notifications alphabetically' },
  ];

  return (
    <div class="section">
      <h2>Repo Dashboard</h2>
      <p class="hint">Configure default views and sorting for the Repo Dashboard tab.</p>

      <div class="settings-grid">
        <div class="settings-group">
          <label>Default View</label>
          <select
            value={prefs.dashboardDefaultFilter ?? 'all'}
            onChange={(e) => setPrefs({ ...prefs, dashboardDefaultFilter: e.currentTarget.value as DashboardCardFilter })}
            class="settings-select"
          >
            {cardFilterOptions.map((opt) => (
        ...
        <div class="settings-group">
          ...
          <select
            value={prefs.dashboardSortMode ?? 'attention'}
            onChange={(e) => setPrefs({ ...prefs, dashboardSortMode: e.currentTarget.value as DashboardSortMode })}
            class="settings-select"
          >
            {sortModeOptions.map((opt) => (
              <option key={opt.value} value={opt.value} title={opt.description}>
                {opt.label}
              </option>
            ))}
        ...
        <div class="settings-group">
          <label>Notification Sort</label>
          <select
            value={prefs.dashboardNotifSort ?? 'count'}
            onChange={(e) => setPrefs({ ...prefs, dashboardNotifSort: e.currentTarget.value as DashboardNotifSort })}
            class="settings-select"
          >
            {notifSortOptions.map((opt) => (
              <option key={opt.value} value={opt.value} title={opt.description}>
                {opt.label}
              </option>
            ))}
      ...
      <div class="btn-row" style={{ marginTop: '0.75rem' }}>
        <button class="btn-save" onClick={handleSave} disabled={saving}>
          {saving ? 'Saving...' : saved ? '\u2713 Saved' : 'Save Dashboard Settings'}
        </button>
      </div>
    </div>
  );
